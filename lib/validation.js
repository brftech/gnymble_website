...
export const validateDemoForm = (data) => {
  ...
  // Check for either message or useCase (demo form sends message, API expects useCase)
  if (!validateRequired(data.message) && !validateRequired(data.useCase)) {
    errors.message = 'Tell us about your needs is required';
  }
  ...
  // Validate message/useCase length (demo form sends message, API expects useCase)
  const messageContent = data.message || data.useCase;
  if (messageContent && !validateLength(messageContent, 10, 1000)) {
    errors.message = 'Tell us about your needs must be between 10 and 1000 characters';
  }
...
export const sanitizeDemoForm = (data) => {
  return {
    ...
    useCase: sanitizeString(data.message || data.useCase), // Handle both message and useCase
